...
func getCertificateMap(t *testing.T) map[string][]byte {
...
func Test_Postgres_Provider_End2End_Tls_From_Secret(t *testing.T) {
	...
	filledForm.TLS.TLSModeFromSecret.TLSSecret = "pg-test-cluster-tls-secret"
	filledForm.TLS.TLSModeFromSecret.CaSecret = "pg-test-cluster-tls-secret"

	Postgres_Provider_End2End(t, pgProviderPtr, user, filledForm)

	err = api.DeleteSecret(user.GetKubernetesNamespace(), "pg-test-cluster-tls-secret")
	assert.Nil(t, err)
}
